...
def find_at_advantage_square(line, board)
...
WINNING_LINES.each do |line|
    square = find_at_advantage_square(line, brd)
    break if square
  end
...
#   WINNING_LINES.each do |line|
#   #   #binding.pry
#   #   if brd.values_at(*line).count(PLAYER_MARKER) == 2 && brd.has_value?(INITIAL_MARKER) == true
