...
def whos_turn?
  players = []
      if FIRST_TURN == "choose"
        prompt("Would you like play as the first?[y/n]")
        first_answer = gets.chomp.downcase
        if first_answer.start_with?("y")
          players.unshift("player","computer")
        else
          players.unshift("computer", "player")
        end  
      end 
end
...
def place_piece!(brd, players)
  if players[0] == "player"
        player_places_piece!(brd)
      elsif players[0] == "computer"
